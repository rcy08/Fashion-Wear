...
def managerhomepage():
    return render_template("managerhomepage.html")
...
def orderhistory():
    cur = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cur.execute(
        "SELECT orderid,resname,item,quantity,price,orderdate,status FROM orders where username=%s and status!=%s",
        (session["username"], "ordered"),
    )
    result = cur.fetchall()
    if result:
...
def feedback(id):
    ...
    if request.method == "POST" and "rating" in request.form:
        ...
        return render_template("rating.html", item=item)
...
@app.route("/cancelorderm/<string:id>", methods=["POST", "GET"])
def cancelorderm(id):
    msg = ""
    cur = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cur.execute("SELECT * FROM orders WHERE orderid=%s", ([id]))
    ...
    if status == "confirmed":
        ...
        result = cur.fetchall()
        return render_template(
            "orderhistorymanager.html", detail=result, msg = "Order Cancelled"
        )
    return redirect(url_for('orderhist'))
    # else:
    #     if status == "ordered":
    #         cur = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    #         cur.execute(
...
def orderhist():
    cur = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cur.execute(
        "SELECT username,item,quantity,price,orderdate,status,orderid,address FROM orders where resname = %s",
        (session["resname"],),
    )
    result = cur.fetchall()
    print(result)
